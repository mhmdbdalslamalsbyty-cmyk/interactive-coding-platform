...
function setView(name){
  ...
  document.body.classList.remove('menu-open'); $('#menu-button').setAttribute('aria-expanded', 'false'); window.scrollTo({top:0, behavior:'smooth'});
}
...
function openLesson(id){
  ...
  $('#editor-label').textContent = `تجربة ${current.track}`; $('#runtime-label').textContent = current.track === 'HTML & CSS' ? 'معاينة مباشرة' : current.track === 'Python' ? 'Python في المتصفح' : 'تطبيق محلي';
...
function runPython(code){ const output = $('#code-output'); output.textContent = 'جارٍ تشغيل Python…'; $('#html-preview').hidden = true; try { const lines=[]; const oldWrite=window.__alsabtaniWrite; window.__alsabtaniWrite=(text)=>lines.push(String(text)); const wrapped=`from browser import window\nclass Capture:\n def write(self, text):\n  getattr(window, "__alsabtaniWrite")(text)\n def flush(self):\n  pass\nimport sys\nsys.stdout=Capture()\nsys.stderr=Capture()\n${code}`; __BRYTHON__.imported.exec={}; eval(__BRYTHON__.py2js(wrapped).to_js()); output.textContent=lines.join('').trim()||'اكتمل التشغيل بدون مخرجات.'; window.__alsabtaniWrite=oldWrite; } catch(error) { output.textContent = `خطأ: ${error.message || error.args?.join(' ') || 'تعذر تنفيذ الكود.'}`; } }
function runCode(){ if (!current) return; const code = $('#code-editor').value; if (current.track === 'HTML & CSS') { const frame = $('#html-preview'); frame.hidden = false; frame.srcdoc = code; $('#code-output').textContent = 'المعاينة تظهر أسفل المحرر.'; } else if (current.track === 'Python') runPython(code); else { $('#html-preview').hidden=true; $('#code-output').textContent = 'محرر C++ تعليمي: انسخ المثال وشغّله محلياً عبر g++ أو في بيئة C++ موثوقة.\n\nالمثال الحالي:\n' + code; } }
...
function showCertificate(track){ if (!trackDone(track)) { toast(`أكمل محطات ${track} أولاً لفتح الشهادة.`); return; } $('#certificate-track').textContent=track; $('#certificate-date').textContent=new Intl.DateTimeFormat('ar-YE',{year:'numeric',month:'long',day:'numeric'}).format(new Date()); $('#certificate-modal').hidden=false; }
